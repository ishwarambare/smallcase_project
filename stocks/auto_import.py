...
def auto_import_stocks():
    ...
    and bulk imports them. Then runs a bulk price update for ALL stocks missing prices.
    ...
    try:
        ...
        # Fetch prices for ALL stocks currently missing a price (new + pre-existing)
        from .utils import update_stock_prices_bulk
        missing_price_symbols = list(
            Stock.objects.filter(current_price__isnull=True).values_list('symbol', flat=True)
        )

        if missing_price_symbols:
            print(f"[AutoImport] Fetching prices for {len(missing_price_symbols)} stocks missing prices...")
            update_stock_prices_bulk(missing_price_symbols)
            print("[AutoImport] Price fetch completed.")
        else:
            print("[AutoImport] All stocks already have prices. Skipping price fetch.")
